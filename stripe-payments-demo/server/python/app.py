...
static_dir = '/home/np108/hosted_sites/aolgifts_python/stripe-payments-demo/public'
...
def serve_js(path):
    ...
    return send_from_directory('/home/np108/hosted_sites/aolgifts_python/stripe-payments-demo/public/javascripts', path)


@app.route('/stylesheets/<path:path>', methods=['GET'])
def serve_css(path):
    #return send_from_directory(f'{static_dir}/stylesheets', path)
    return send_from_directory('/home/np108/hosted_sites/aolgifts_python/stripe-payments-demo/public/stylesheets', path)


@app.route('/images/<path:path>', methods=['GET'])
def serve_image(path):
    return send_from_directory('/home/np108/hosted_sites/aolgifts_python/stripe-payments-demo/public/images', path)
...
def get_config():
    return jsonify({
        ...
        'stripeCountry': os.getenv('STRIPE_ACCOUNT_COUNTRY') or 'CA',
        'country': 'CA',
        'currency': 'cad',
...
def webhook_received():
    ...
    if data_object['object'] == 'source' \
        ...
        source = data_object
        print('Webhook received! The source {source["id"]} is chargeable')
        ...
        if payment_intent['status'] != 'requires_payment_method':
            return jsonify({'error': 'PaymentIntent already has a status of {payment_intent["status"]}'}), 403
